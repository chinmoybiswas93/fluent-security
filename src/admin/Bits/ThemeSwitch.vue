 ...
 * this plugin's, on purpose. Both plugins sit in the same admin menu, and somebody who has
 * chosen dark in one has chosen it for both - sharing the key is what makes that true
 * without either plugin having to know the other is installed. Switching here moves an open
 * FluentCart tab, and vice versa.
 *
...
<template>
    <el-dropdown trigger="click" @command="select" popper-class="fls_theme_menu">
        <button type="button" class="fls_theme_trigger" :title="label" :aria-label="label"
        ...
        <template #dropdown>
            <el-dropdown-menu>
                <el-dropdown-item v-for="option in options" :key="option.value"
                    ...
                    <span class="fls_theme_item">
                        <span class="fls_theme_item_icon" v-html="icons[option.value]"></span>
